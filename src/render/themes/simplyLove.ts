...
const ITG_JUDGMENT: Record<number, JudgmentStyle> = {
...
function font(ds: number, w: number, px: number): string {
...
export class SimplyLoveTheme implements Theme {
  ...
  drawTapNote(
    ...
    traceCel(ctx, s, CEL_FACE);
    ctx.fillStyle = style === 'deadHead' ? '#7c8087' : this.quantColor[quant];
    ctx.fill();
  ...
  drawExplosion(
    ...
    traceCel(ctx, v.arrowS, CEL_OUTLINE);
    ctx.fillStyle = color;
    ctx.globalAlpha = 0.5 * fade;
    ctx.fill();
    ctx.shadowBlur = 0;
    traceCel(ctx, v.arrowS * 0.88, CEL_OUTLINE);
    ctx.fillStyle = '#ffffff';
