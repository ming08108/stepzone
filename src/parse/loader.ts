...
/** Parse a simfile's text into a Song. `filename` is optional but improves detection. */
export function parseSimfile(text: string, filename?: string): Song {
  return detectFormat(text, filename) === 'ssc' ? parseSsc(text) : parseSm(text);
}
