...
describe('parseSubmitScoreRequest', () => {
  ...
  it('pins the current protocol version', () => {
    ...
    expect(validSubmit().protocol).toBe(4);
  });
  ...
  it('rejects non-objects and wrong protocol versions', () => {
    ...
    // queued plays are dropped on load rather than accepted without current evidence.
  ...
  it('requires a chart and replay for authoritative scoring', () => {
    ...
    // The chart itself is required (the server re-simulates against it).
