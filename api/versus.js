...
function randomRoomCode() {
  ...
  return Array.from(bytes, (b2) => CODE_ARROWS[b2 % 4]).join("");
}
function isRoomCode(v2) {
  return typeof v2 === "string" && new RegExp(`^[LDUR]{${CODE_LENGTH}}$`).test(v2);
}
var isObj2 = (v2) => typeof v2 === "object" && v2 !== null && !Array.isArray(v2);
...
function parseSignalRequest(v2) {
  ...
  if (v2.t === "create") {
    ...
    if (!chart || !name(v2.hostName) || !sdp(v2.offer)) return null;
    if (!num(v2.musicRate) || v2.musicRate < 0.5 || v2.musicRate > 3) return null;
    return { t: "create", hostName: v2.hostName, chart, musicRate: v2.musicRate, offer: v2.offer };
  }
  if (v2.t === "answer") {
...
function createSignalHandlers(store, now = Date.now) {
  return {
    async GET(req) {
...
var SCHEMA = `CREATE TABLE IF NOT EXISTS net_versus_rooms (
...
// src/net/versusEntry.ts
var databaseUrl = process.env.DATABASE_URL;
var handlers = databaseUrl ? createSignalHandlers(new PgSignalStore(databaseUrl)) : null;
...
var GET = handlers ? handlers.GET : unavailable;
var POST = handlers ? handlers.POST : unavailable;
export {
  GET,
  POST
};
