...
function bearerToken(req) {
  ...
  const m2 = /^Bearer\s+(.+)$/i.exec(h.trim());
  return m2 ? m2[1].trim() : null;
}
function tokenEquals(a2, b2) {
  if (a2.length !== b2.length) return false;
  let diff = 0;
  for (let i = 0; i < a2.length; i++) diff |= a2.charCodeAt(i) ^ b2.charCodeAt(i);
  return diff === 0;
}
...
function parsePush(raw) {
  ...
  if (o.metrics && typeof o.metrics === "object") {
    let n = 0;
    for (const [k, v2] of Object.entries(o.metrics)) {
      if (n >= MAX_METRICS) break;
      if (isFiniteNum(v2)) {
        metrics[k.slice(0, 48)] = v2;
        n++;
      }
    }
  }
  ...
  if (o.box && typeof o.box === "object") {
    const b2 = o.box;
    box = {
      gpu: typeof b2.gpu === "string" ? b2.gpu.slice(0, 120) : "GPU",
...
function createPushHandler(store, token2, now = Date.now) {
  ...
  return {
    async POST(req) {
      ...
      if (!supplied || !tokenEquals(supplied, token2)) {
...
// src/net/experimentsPushEntry.ts
...
var handler = token ? createPushHandler(
  databaseUrl ? new PgExperimentsStore(databaseUrl) : new MemoryExperimentsStore(),
  token
) : null;
var POST = handler ? handler.POST : unavailable;
export {
  POST
};
