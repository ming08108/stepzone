// GENERATED by scripts/buildApi.mjs — edit src/net/*Entry.ts, not this file.

// src/net/httpResponse.ts
...
var finiteNum = (v2) => typeof v2 === "number" && Number.isFinite(v2);
var str = (v2, max) => typeof v2 === "string" && v2.length > 0 && v2.length <= max;
function parseCounts(v2) {
...
function parseChartRef(v2) {
  if (!isObj(v2)) return null;
  if (!str(v2.chartHash, MAX_ID_LENGTH)) return null;
  if (!str(v2.title, MAX_STRING_LENGTH) || typeof v2.artist !== "string") return null;
  if (v2.artist.length > MAX_STRING_LENGTH) return null;
  if (!str(v2.stepsType, MAX_STRING_LENGTH)) return null;
  if (!finiteNum(v2.difficulty) || !Number.isInteger(v2.difficulty)) return null;
  if (!finiteNum(v2.meter) || !Number.isInteger(v2.meter) || v2.meter < 0 || v2.meter > 100)
    return null;
  return {
...
function parsePlayResult(v2) {
  if (!isObj(v2)) return null;
  if (!finiteNum(v2.percent) || v2.percent < 0 || v2.percent > 1) return null;
  if (!str(v2.grade, 8)) return null;
  if (!finiteNum(v2.maxCombo) || !Number.isInteger(v2.maxCombo)) return null;
  if (v2.maxCombo < 0 || v2.maxCombo > MAX_COUNT) return null;
  if (typeof v2.failed !== "boolean") return null;
  const counts = parseCounts(v2.counts);
  const holdCounts = parseCounts(v2.holdCounts);
  if (!counts || !holdCounts) return null;
  return {
    percent: v2.percent,
    grade: v2.grade,
    maxCombo: v2.maxCombo,
    failed: v2.failed,
    counts,
    holdCounts
  };
}
var MAX_GHOST_FRAMES = 1e3;
function parseGhost(v2) {
  if (!Array.isArray(v2) || v2.length > MAX_GHOST_FRAMES) return null;
  const out = [];
  let prevAt = Number.NEGATIVE_INFINITY;
  for (const f of v2) {
    if (!isObj(f)) return null;
    if (!finiteNum(f.atSong) || f.atSong < -60 || f.atSong > 36e3) return null;
    if (f.atSong < prevAt) return null;
...
function parseSubmitScoreRequest(v2) {
  ...
  if (!str(v2.playerId, MAX_ID_LENGTH) || !str(v2.secret, MAX_ID_LENGTH)) return null;
  if (!str(v2.playerName, MAX_NAME_LENGTH)) return null;
  const chart = parseChartRef(v2.chart);
  const result = parsePlayResult(v2.result);
  if (!chart || !result) return null;
  if (!finiteNum(v2.musicRate) || v2.musicRate < 0.5 || v2.musicRate > 3) return null;
  let judged = 0;
...
function createHandlers(store, now = Date.now) {
  return {
    async GET(req) {
...
// src/net/scoreStore.ts
function mergeStoredBest(prev, req, now) {
  const r = req.result;
...
function compareBests(a2, b2) {
  if (a2.result.percent !== b2.result.percent) return b2.result.percent - a2.result.percent;
  return a2.updatedAt - b2.updatedAt;
}
var MemoryScoreStore = class {
  players = /* @__PURE__ */ new Map();
  boards = /* @__PURE__ */ new Map();
  /** Chart metadata by hash (display only; last write wins). */
  charts = /* @__PURE__ */ new Map();
  board(chartHash, rk) {
    const key = `${chartHash}\xB7${rk}`;
    let b2 = this.boards.get(key);
    if (!b2) {
      b2 = /* @__PURE__ */ new Map();
      this.boards.set(key, b2);
    }
    return b2;
  }
  submit(req, secretHash, now) {
...
var PgScoreStore = class {
  ...
  async submit(req, secretHash, now) {
    ...
    const prevRows = prevRowsRaw;
    const { next, isPersonalBest } = mergeStoredBest(
      prevRows[0] ? toStoredBest(prevRows[0]) : void 0,
      req,
      now
    );
    await sql.query(
  ...
  async top(chartHash, rate, limit) {
    await this.ensureSchema();
  ...
  async ghost(chartHash, rate, playerId) {
    await this.ensureSchema();
    const rows = await this.sql.query(
