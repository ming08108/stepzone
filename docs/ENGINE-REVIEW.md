...
| **Combo governance** | ⚠️ | We count **per note** (each W3+ grows, <W3 breaks). The engine uses **chord cohesion** (per row; the worst-timed tap governs the whole row). Final combo and dance-% agree for streams and for jumps that break combo; they can differ only in `maxCombo` on a jump with *mixed* W1+low scores (rare). Dance points are per-note in both. |
